% $Id: parimacro.tex,v 1.10.2.2 2005/09/15 14:58:19 bill Exp $
...
\else     %NO
  ...
  \checkfile{wantpdfoutput} % check if we want PDF output
  \ifeof\std \else \PDFtrue \fi
\fi
...
\ifGPHELP % disable crossreferences
  ...
  \def\tocwrite#1{}
  \def\label#1{}
  \def\ref#1{[{\bf ??}]} % all crossref marked as [??]
...
%% If PDF, associate the page to a unique integer (\pdfdestcntr).
  ...
  \checkfile{users.aux}% don't use \jobname: may be used by tutorial.tex
  \ifeof\std
    \message{No aux file.}
  \else
...
%Table of contents. cf TeXBook Exercice 21.10
