% $Id: refcard.tex,v 1.7.2.1 2005/09/15 14:58:19 bill Exp $
% This file is intended to be processed by plain TeX (TeX82).
% Reference Card for PARI-GP version 2.1

% Copyright (c) 1997-2000 Karim Belabas.
...
%  F-91405 Orsay
%  Internet: Karim.Belabas@math.u-psud.fr
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Note: optional arguments are surrounded by braces {\tt \{\}}.
...
\section{Help}
\li{describe function}{?{\rm function}}
\li{extended description}{??{\rm keyword}}
\li{list of relevant help topics}{???{\rm pattern}}
...
\key{extend statements on several lines}{\{\seq1; \seq2;\}}
\key{comment}{/* \dots */}
...
\key{enable/disable logfile}{\\l \{filename\}}
...
\li{read file into GP}{\\r {\rm filename}}
\li{write \%$n$ to file}{\\w $n$ {\rm filename}}

\section{GP Within Emacs}
\li{to enter GP from within Emacs:}{M-x gp{\rm,} C-u M-x gp}
\li{word completion}{<TAB>}
...
\li{\typ{COMPLEX}. Complex Numbers}{$x+\kbd{I}*y$}
...
\li{\typ{VEC}/\typ{COL}. Row/Column Vectors}
  {$[x,y,z]${\rm,} $[x,y,z]$\til}
%\li{\typ{COL}. Column Vectors}{$[x,y,z]$\til}
...
\li{make $x$ a vector, matrix, set, list, string}
  {Vec{\rm,}Mat{\rm,}Set{\rm,}List{\rm,}Str}
...
\li{error recovery (try \seq1)}{trap$(\{err\},\{\seq2\},\{\seq1\})$}
%
\subsec{Input/Output}
\li{prettyprint args with/without newline}{printp(){\rm,} printp1()}
\li{print args with/without newline}{print(){\rm,} print1()}
...
\li{adjoint of the matrix $x$}{matadj$(x)$}
...
\li{vector of elementary divisors of $x$}{matsnf$(x)$}
...
\li{interpolating poly evaluated at $a$}{polinterpolate$(X,\{Y\},\{a\},\{\&e\})$}
...
\li{gcd of $x$ and $y$}{gcd$(x,y)$}
\li{LCM of $x$ and $y$}{lcm$(x,y)$}
...
\li{multiplicative order of $x$ (intmod)}{znorder$(x)$}
...
%
Elliptic curve initially given by $5$-tuple $E=$\kbd{[a1,a2,a3,a4,a6]}.
...
\endindentedkeys
\li{change curve $E$ using $v=[u,r,s,t]$}{ellchangecurve$(ell,v)$}
\li{change point $z$ using $v=[u,r,s,t]$}{ellchangepoint$(z,v)$}
\li{cond, min mod, Tamgawa nmbr $[N,v,c]$}{ellglobalred$(ell)$}
\li{Kodaira type of $p$ fiber of $E$}{elllocalred$(ell,p)$}
\li{add points $z1+z2$}{elladd$(ell,z1,z2)$}
\li{subtract points $z1-z2$}{ellsub$(ell,z1,z2)$}
...
\li{canonical bilinear form taken at $z1$, $z2$}{ellbil$(ell,z1,z2)$}
...
\li{copy $w$ to $w2$ with offset $(dx,dy)$}{plotcopy$(w,w2,dx,dy)$}
...
\li{reduce $x$ ($s =\sqrt{D}$, $l=\lfloor s\rfloor$)}
  {qfbred$(x,\fl,\{D\},\{l\},\{s\})$}
...
\key{number of [real,complex] places}{\var{nf}.sign}
...
%
\li{init big number field structure \var{bnf}}{bnfinit$(f,\fl)$}
...
\key{$[tu,fu]$, $[fu,tu]$}{\var{bnf}.tufu{\rm/}futu}
\endindentedkeys
\li{compute a \var{bnf}\ from small \var{bnf}}{bnfmake$(\var{sbnf})$}
%
\li{add $S$-class group and units, yield \var{bnfs}}{bnfsunit$(\var{nf},S)$}
...
\li{roots of $g$ belonging to {\tt nf}}{nfroots$(\var{nf},g)$}
\li{factor $g$ in {\tt nf}}{nffactor$(\var{nf},g)$}
\li{factor $g$ mod prime $pr$ in {\tt nf}}{nffactormod$(\var{nf},g,pr)$}
\li{number of roots of $1$ in {\tt nf}}{nfrootsof1$(nf)$}
\li{conjugates of a root $\theta$ of {\tt nf}}{nfgaloisconj$(\var{nf},\fl)$}
\li{apply Galois automorphism $s$ to $x$}{nfgaloisapply$(\var{nf},s,x)$}
\li{subfields (of degree $d$) of {\tt nf}}{nfsubfields$(\var{nf},\{d\})$}
%
...
\section{Class Groups \& Units (\var{bnf}, bnr)}
\leavevmode
$a1,\{a2\},\{a3\}$ usually $bnr,subgp$ or $\var{bnf},module,\{subgp\}$
\hfill\break
%
...
\li{discriminant of class field ext}{bnrdisc$(a1,\{a2\},\{a3\})$}
...
\li{is $id$ an ideal in {\tt nf}?}{nfisideal$(\var{nf},id)$}
\li{is $x$ principal in {\tt bnf}?}{bnfisprincipal$(\var{bnf},x)$}
...
\li{factor ideal $x$ in {\tt nf}}{idealfactor$(\var{nf},x)$}
\li{recover $x$ from its factorization in {\tt nf}}{factorback$(x,nf)$}
\li{decomposition of prime $p$ in {\tt nf}}{idealprimedec$(\var{nf},p)$}
\li{valuation of $x$ at prime ideal $pr$}{idealval$(\var{nf},x,pr)$}
\li{weak approximation theorem in {\tt nf}}{idealchinese$(\var{nf},x,y)$}
...
\li{is $x\in K$ a norm from $L$?}{rnfisnorm$(\var{bnf},ext,x,\fl)$}
\vfill
\copyrightnotice
\bye
% Local variables:
